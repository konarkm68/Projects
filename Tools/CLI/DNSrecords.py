...
class DNSrecords:
    def get_domain():
        ...
        sub_domain = input(
            f"{Style.RESET_ALL}Enter Sub-Domain (Press 'Enter' for root domain search'): {Fore.YELLOW}"
        )
        ...
        for record in record_types:
            if record == "TXT":
                ...
                for sec_email in sec_email_TXT_DNS:
                    try:
                        sec_email_result = dns.resolver.resolve(
                            sec_email + root_domain, "TXT"
                        )
                        for sec_email_txt in sec_email_result:
                            print(
                                "TXT-Secure Email",
                                f"{Fore.BLUE}{sec_email_txt.to_text()}",
                            )
                    except:
                        continue
                print()
