    ...
    def NumSys_converter():
        ...
        match choice:
            case 1:
                ...
                print(
                    f"{Fore.BLUE}The Binary-Number is{':':>8} {num:b}"
                )  # bin(num)[2:]
                print(f"{Fore.BLUE}The Octal-Number is{':':>9} {num:o}")  # oct(num)[2:]
                print(
                    f"{Fore.BLUE}The Hexa-Decimal--Number is{':':0} {num:x} OR {num:X}"
                )  # hex(num)[2:]
            case 2:
            ...
            case 4:
                num = int(
                    input(f"Enter the Hexa-Decimal--Number{':':0} {Fore.YELLOW}"), 16
                )
...
\t        c. EXIT!"""
        ...
        def BinaryAdd():
            ...
            num = int(
                input(f"{Fore.YELLOW}Enter the number of binary-numbers to add: ")
            )

            for i in range(num):
                bin_num = input(
                    f"{Style.RESET_ALL}Enter Binary-Number #{i+1}: {Fore.YELLOW}"
                )
                if i == 0:
    ...
    def complement():
        ...
        bits = int(
            input(f"Enter the number of bits (including signed-bit): {Fore.YELLOW}")
        )
        bits_mask = int(bin(2**bits - 1)[2:], 2)

        num = int(
            input(
                f"{Style.RESET_ALL}Enter the ({bits}-bit-Binary-Number) = (1-signed-bit + {bits-1}-Binary-Number-bits): {Fore.YELLOW}"
            ),
            2,
        )
