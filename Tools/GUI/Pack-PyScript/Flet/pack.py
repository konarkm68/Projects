...
def main(page: flet.Page):
    ...
    def pack(py_script, exe_name):
        try:
            ...
            pyexe_path = rf"C:\Users\{user_name}\Documents\PyInstaller"

            os.mkdir(rf"{pyexe_path}")
            os.mkdir(rf"{pyexe_path}\dist")
            os.mkdir(rf"{pyexe_path}\build")
            os.mkdir(rf"{pyexe_path}\spec")
        ...
        try:
            ...
            if not exe_name.value or exe_name.value == "":
                page.snack_bar.content = flet.Text(
                    f"Error! - Please select a Python-Script!",
                    size=10,
                    color="red",
                    weight="bold",
                )
            ...
            if result == None:
                ...
                page.snack_bar.content = flet.Text(
                    f"Success! - Your .exe file is located in Documents/PyInstaller/dist/ OR ./{exe_name.value}",
                    size=10,
                    color="green",
                )
                page.snack_bar.action = "Open Directory!"
                page.snack_bar.on_action = lambda _: os.startfile(
                    r"C://Users//{}//Documents//PyInstaller//dist".format(os.getlogin())
                )
                page.snack_bar.open = True
                page.update()
            ...
            page.snack_bar.content = flet.Text(
                f"Error! - {e}", size=10, color="red", weight="bold"
            )
            page.snack_bar.open = True
            page.update()
    ...
    exe_name = flet.TextField(
        width=185,
        height=60,
        max_length=15,
        hint_text="Default: PyScript-Name",
        text_size=12,
    )
    ...
    def select_ico(e: flet.FilePickerResultEvent):
        try:
            ...
            page.snack_bar.content = flet.Text(
                f"Error! - Please select an Icon-File!",
                size=10,
                color="red",
                weight="bold",
            )
    ...
    pack_btn = flet.FilledButton(
        "Pack PyScript! (.py --> .exe)",
        on_click=lambda _: pack(py_script, exe_name),
        width=400,
        height=50,
    )
    ...
    page.snack_bar = flet.SnackBar(
        open=False, duration=15000, content=flet.Text(""), show_close_icon=True
    )
    ...
    page.add(
        flet.Column(
            [
                ...
                flet.Row(
                    [
                        flet.Text("--console ?", width=90, font_family="consolas"),
                        console,
                        flet.Text("--icon ?", width=100, font_family="consolas"),
                        icon,
                    ]
                ),
                FileDIR,
                ...
                flet.Row(
                    [
                        flet.Text("exe-name ?", width=85, font_family="consolas"),
                        exe_name,
                    ]
                ),
                pack_btn,
