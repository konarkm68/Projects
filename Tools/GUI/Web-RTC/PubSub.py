...
def main(page: flet.Page):
    ...
    def join_chat_click(e):
        if not join_user_name.value:
            ...
            join_user_name.value = join_user_name.value.capitalize()
    ...
    def clear_chat():
        ...
        page.pubsub.send_all(
            Message(
                page.session.get("user_name"),
                f"{page.session.get('user_name')} has cleared the chat.",
                message_type="log_msg",
            )
        )
    ...
    def save_chat():
        ...
        page.pubsub.send_all(
            Message(
                page.session.get("user_name"),
                f"{page.session.get('user_name')} has saved the chat.",
                message_type="log_msg",
            )
        )
    ...
    def exit_app():
        page.pubsub.send_all(
            Message(
                page.session.get("user_name"),
                f"{page.session.get('user_name')} has left the chat.",
                message_type="log_msg",
            )
        )
        page.window_close()
    ...
    send_button = flet.IconButton(
        icon=flet.icons.ADD_IC_CALL_ROUNDED,
        tooltip="Join Text-Chat",
    ...
    options = flet.PopupMenuButton(
        ...
        items=[
            ...
            flet.PopupMenuItem(
                text="Leave Chat",
                on_click=lambda e: exit_app(),
            ),
            flet.PopupMenuItem(
                text="Save Chat",
                on_click=lambda e: save_chat(),
            ),
        ],
    )
